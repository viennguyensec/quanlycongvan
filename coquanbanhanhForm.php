...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if(empty($ten_err)){
    if(isset($_POST["id"]) && !empty($_POST["id"])) {
      $sql = "UPDATE coquanbanhanh SET ten=?, mota=? WHERE id=?";
    } else {
      $sql = "INSERT INTO coquanbanhanh (ten, mota) VALUES (?, ?)";
    }
    ...
    if($stmt = mysqli_prepare($connect, $sql)){
      ...
      if(mysqli_stmt_execute($stmt)){
        header("location: coquanbanhanh.php");
        exit();
      } else{
  ...
  if(isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
    $id = trim($_GET["id"]);
    $sql = "SELECT * FROM coquanbanhanh WHERE id = ?";
    if ($stmt = mysqli_prepare($connect, $sql)) {
    ...
    <div class="col-sm-8 col-lg-8">
      <div class="panel panel-default">
        <div class="panel-heading">
          <i class="fa fa-wechat"></i> <?php echo (isset($_GET["id"]) && !empty(trim($_GET["id"]))) ? 'Chỉnh sửa Cơ quan ban hành' : 'Tạo mới Cơ quan ban hành' ?>
        </div>
        <div class="panel-body">
          <div class="table-responsive">
            <div class="row">
              <div class="col-lg-12">
                <form action="coquanbanhanhForm.php" id="form-create-coquanbanhanh" method="post">
                  <?php
                    if (isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
