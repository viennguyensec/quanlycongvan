...
$userId = $kyhieu = $ngaybanhanh = $noinhan = $loaiId = $nguoigui = $nguoiky = $nguoiduyet = $noidung = $file = "";
$userId_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if(empty($userId_err)){
    ...
    if(isset($_POST["id"]) && !empty($_POST["id"])) {
      $sql = "UPDATE congvandi SET kyhieu=?, ngaybanhanh=?, noinhan=?, loaiId=?, nguoigui=?, nguoiky=?, nguoiduyet=?, noidung=?, file=? WHERE id=?";
    } else {
      $sql = "INSERT INTO congvandi (userId, kyhieu, ngaybanhanh, noinhan, loaiId, nguoigui, nguoiky, nguoiduyet, noidung, file) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    }

    if($stmt = mysqli_prepare($connect, $sql)){
      if(isset($_POST["id"]) && !empty($_POST["id"])) {
        mysqli_stmt_bind_param($stmt, "ssiisssssi", $param_kyhieu, $param_ngaybanhanh, $param_noinhan, $param_loaiId, $param_nguoigui, $param_nguoiky, $param_nguoiduyet, $param_noidung, $param_file, $param_id);
        $param_id = $_POST["id"];
      } else {
        mysqli_stmt_bind_param($stmt, "issiisssss", $param_userId, $param_kyhieu, $param_ngaybanhanh, $param_noinhan, $param_loaiId, $param_nguoigui, $param_nguoiky, $param_nguoiduyet, $param_noidung, $param_file);
      }

      $param_userId = $_SESSION['userId'];
      $param_kyhieu = $kyhieu;
      $param_ngaybanhanh = $ngaybanhanh;
      $param_noinhan = $noinhan;
      $param_loaiId = $loaiId;
      $param_nguoigui = $nguoigui;
  ...
  if(isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
    $id = trim($_GET["id"]);
    $sql = "SELECT * FROM congvandi WHERE id = ?";
    if ($stmt = mysqli_prepare($connect, $sql)) {
    ...
    <div class="col-sm-8 col-lg-8">
      <div class="panel panel-default">
        <div class="panel-heading">
          <i class="fa fa-wechat"></i> <?php echo (isset($_GET["id"]) && !empty(trim($_GET["id"]))) ? 'Chỉnh sửa công văn đi' : 'Tạo mới công văn đi' ?>
        </div>
        <div class="panel-body">
          <div class="table-responsive">
            <div class="row">
              <div class="col-lg-12">
                <form action="congvandiForm.php" id="form-create-congvandi" method="post" enctype="multipart/form-data">
                  <?php
                    if (isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
                  ...
                  <div class="form-group <?php echo (!empty($noinhan_err)) ? 'has-error' : ''; ?>">
                    <label>Nơi nhận</label>
                    <input type="date" name="noinhan" class="form-control" value='<?php echo $noinhan ?>' />
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" name="nguoigui" class="form-control" value='<?php echo $nguoigui ?>' />
                  </div>
                  <div class="form-group">
